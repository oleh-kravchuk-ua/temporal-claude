...
- **Temporal owns retries:** the client uses `maxRetries: 0` and a timeout of `ACTIVITY_START_TO_CLOSE_MS` (1 minute, `workflow/activity-timeout.ts`) minus 15 s, so a hung call ends as a retryable timeout before the attempt does. The retry policy is `ACTIVITY_RETRY` in the same file: 4 attempts, waiting 2 s, 4 s, 8 s (cap 30 s), so a short outage is survived instead of failing in seconds; a hung call takes at most about 3.2 min (4 × 45 s + 14 s). `claude-errors.ts` turns transient API errors (429, 5xx incl. 529, 409) into retryable `ApplicationFailure`s with a sanitized message and, when the server sent `retry-after`, that as `nextRetryDelay` (clamped to 1–60 s), so a rate limit is waited out; connection errors and timeouts pass through unchanged. Permanent ones (400/401/403/404/422, refusal, truncation, empty output) become non-retryable `ApplicationFailure`s. Failure messages never copy the upstream message.
